...
def get_ticket_action_keyboard(ticket_id: int, is_staff: bool = False) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    if is_staff:
        # Для персонала: приоритет и закрыть
        builder.row(
            InlineKeyboardButton(text="⚡ Приоритет", callback_data=TicketCallback(action="change_priority", ticket_id=ticket_id).pack()),
            InlineKeyboardButton(text="🔒 Закрыть", callback_data=TicketCallback(action="close", ticket_id=ticket_id).pack()),
            width=2
        )
    else:
        # Для пользователя: только ответить
        builder.row(
            InlineKeyboardButton(text="📝 Ответить", callback_data=TicketCallback(action="add_message", ticket_id=ticket_id).pack()),
        )
    builder.row(InlineKeyboardButton(text="⬅️ Назад", callback_data="back_to_tickets"))
    return builder.as_markup()

def get_reply_to_ticket_keyboard(ticket_id: int) -> InlineKeyboardMarkup:
    """Клавиатура для ответа на тикет из уведомления"""
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="📝 Ответить", callback_data=TicketCallback(action="add_message", ticket_id=ticket_id).pack()))
    return builder.as_markup()
