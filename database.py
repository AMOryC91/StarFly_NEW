...
def rate_ticket(ticket_id: int, user_id: int, agent_id: int, rating: int, comment: str = None):
    ...
    cursor = conn.cursor()
    try:
        # Используем INSERT OR IGNORE, чтобы избежать ошибки при повторной оценке
        cursor.execute(
            "INSERT OR IGNORE INTO ticket_ratings (ticket_id, user_id, agent_id, rating, comment) VALUES (?, ?, ?, ?, ?)",
            (ticket_id, user_id, agent_id, rating, comment)
        )
        if cursor.rowcount > 0:
            # Если вставка удалась, обновляем поле rating в tickets
            cursor.execute(
            ...
            # Оценка уже существует – можно обновить (опционально)
            # Либо просто вернуть False, сигнализируя, что оценка уже была
            conn.rollback()
            return False
    except Exception as e:
        conn.rollback()
...
# ========== НОВЫЕ ФУНКЦИИ ДЛЯ СОГЛАШЕНИЯ ==========
def create_agreement_table():
    """Создаёт таблицу для хранения факта принятия пользовательского соглашения, если её нет"""
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('''
...
def has_user_agreed(user_id: int) -> bool:
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT 1 FROM user_agreements WHERE user_id = ?", (user_id,))
    result = cursor.fetchone() is not None
    conn.close()
    return result

def set_user_agreed(user_id: int):
    """Записывает факт принятия соглашения пользователем"""
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        cursor.execute(
            "INSERT OR IGNORE INTO user_agreements (user_id) VALUES (?)",
            (user_id,)
        )
        conn.commit()
    except Exception as e:
        logger.error(f"Ошибка при записи согласия пользователя {user_id}: {e}")
    finally:
        conn.close()