...
async def _process_virtual_screenshot(message: types.Message, state: FSMContext):
    ...
    order_id = create_order(
        user_id=user_id,
        amount=amount,
        recipient_username="self",  # для виртуальной покупки получатель не важен
        screenshot_path=file_path
    )
    # Добавляем комментарий, чтобы отличить виртуальную покупку
    conn = get_db_connection()
    cursor = conn.cursor()
...
async def approve_order(callback: types.CallbackQuery, callback_data: OrderCallback):
    ...
    if order:
        ...
        try:
            bot = callback.bot
            # Если это заказ на виртуальную валюту
            if comment == 'virtual_purchase':
                # Начисляем виртуальные звёзды
